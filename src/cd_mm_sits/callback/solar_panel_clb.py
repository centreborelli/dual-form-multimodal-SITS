...
from cd_mm_sits.lightning_module.supervised_cd import SupervisedCD, SupervisedCDOut


class PlotPredictionsDuringTrainingCallback(Callback):
    ...
    def on_validation_batch_end(
        self,
        trainer: Trainer,
        pl_module: SupervisedCD,
        outputs: SupervisedCDOut,
        batch: CDSupBatch,
        batch_idx: int,
        ...
        if outputs is not None:
            pred = nn.functional.sigmoid(outputs.out).cpu()[..., 0]
...
class SaveTestPrediction(Callback):
    def __init__(
        self,
        num_samples=1,
        device="cpu",
    ...
    def on_test_batch_end(
        ...
        if batch_idx % self.log_every_n_steps != 0:
            return
        pred = nn.functional.sigmoid(outputs.out).detach().cpu().numpy()[0, ...]
        if batch.extracted_ids is not None:
            np.save(
                Path(self.output_dir).joinpath(f"pred_{batch.extracted_ids[0]}.npy"),
                pred,
            )

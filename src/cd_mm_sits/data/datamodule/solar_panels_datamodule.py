"""File which describe the datamodule
for the solar panel data"""
...
from cd_mm_sits.data.dataset.sp_s2_zarr import ZarrS2SolarPanel


class SPDataModule(TemplateDataModule):
    """
    ...
    def setup(self, stage: Literal["fit", "test"]):
        ...
        self.data_train = ZarrS2SolarPanel(
            path_dataset=self.dataset_path,
            list_id=train_data,
            transform=None,
            dataset_type="train",
            max_len=self.max_len_s2,
            crop_size=self.crop_size,
            is_mask_site=self.is_mask_site,
        )
        self.data_val = ZarrS2SolarPanel(
            path_dataset=self.dataset_path,
            list_id=val_data,
            transform=None,
            dataset_type="val",
        ...
        self.data_test = ZarrS2SolarPanel(
            path_dataset=self.dataset_path,
            list_id=test_data,
            transform=None,
            dataset_type="test",
    ...
    def test_dataloader(self):
        return DataLoader(
            ...
            shuffle=False,
            prefetch_factor=self.prefetch_factor,
            collate_fn=collate_fn_sp_test,
        )

    def transfer_batch_to_device(
        self, batch: CDSupBatch, device, dataloader_idx
    ) -> CDSupBatch:
        """
        ...
        """
        if isinstance(batch, CDSupBatch):
    ...
    def on_after_batch_transfer(
        self, batch: CDSupBatch, dataloader_idx: int
    ) -> CDSupBatch:
        """
        ...
        if self.s2_transform.transform is not None:
            sits_s2 = self.s2_transform.transform(batch.sits)
